...
global.fetch = vi.fn(() => {
	return Promise.resolve({
		text() {
			return Promise.resolve(JSON.stringify(API_DATA))
		},
		json() {
...
test('delete should work', async () => {
	...
	await waitFor(() => expect(screen.queryByText(API_DATA[0].name)).toBeInTheDocument())

	await waitFor(() => fireEvent.click(screen.getAllByText(/Not Interested/i)[0]))

	await waitFor(() => expect(screen.queryByText(API_DATA[0].name)).not.toBeInTheDocument())
})
